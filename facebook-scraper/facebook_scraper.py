...
def get_posts(group=None, account=None, pages=None, credentials=None, **kwargs):
    valid_args = sum(arg is not None for arg in (group, account))
    if valid_args != 1:
    ...
    return _get_posts(path, pages, credentials, **kwargs)


def _get_posts(path, pages, timeout=5, sleep=0, credentials=None, extra_info=False):
...
def write_posts_to_csv(group=None, account=None, filename=None, pages=None, **kwargs):
    """
    ...
    """
    list_of_posts = list(get_posts(group=group, account=account, pages=pages, **kwargs))
    ...
    '''
    with open(filename,'r', encoding='utf-8') as f, open(path+'out_final.csv','w', encoding='utf-8') as out_file:
...
def _main():
    ...
    parser.add_argument('-g', '--group', type=str, help="Facebook group")
    parser.add_argument('-a', '--account', type=str, help="Facebook account")
    ...
    write_posts_to_csv(group=args.group, account=args.account, filename=args.filename, pages=args.pages)
